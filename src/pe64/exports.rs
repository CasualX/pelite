...
impl<'a, P: Pe<'a> + Copy> By<'a, P> {
	...
	pub fn iter_names<'s>(&'s self) -> iter::Map<ops::Range<u32>, impl 's + Clone + FnMut(u32) -> (Result<Export<'a>>, Result<&'a CStr>)> {
		(0..self.image.NumberOfNames)
			.map(move |hint| (self.hint(hint as usize), self.name_of_hint(hint as usize)))
	}
}
