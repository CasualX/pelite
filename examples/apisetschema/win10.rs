...
impl<'a> Entries<'a> {
	...
	pub fn iter(&self) -> impl Iterator<Item = Entry<'a>> + Clone + 'a {
		...
		self.entries.iter().map(move |image| Entry { schema, image })
	}
...
impl<'a> Entry<'a> {
	...
	pub fn name(&self) -> Result<&'a [u16]> {
		self.schema.slice_len(self.image.NameOffset, (self.image.NameLength / 2) as usize)
	}
	pub fn values(&self) -> Result<Values<'a>> {
		let image = self.schema.slice_len(self.image.ValueOffset, self.image.ValueCount as usize)?;
