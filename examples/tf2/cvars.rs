...
pub fn cvars<'a>(file: PeFile<'a>) -> pelite::Result<Vec<ConVar<'a>>> {
	...
	while matches.next_match(&mut save) {
		let min_value = None;
		let max_value = None;
		let desc = Some(file.derva_str(save[1]).unwrap().to_str().unwrap());
	...
	while matches.next_match(&mut save) {
		...
		let desc = Some(file.derva_str(save[3]).unwrap().to_str().unwrap());
		let flags = file.derva_copy(save[4]).unwrap();
		let default_string = file.derva_str(save[5]).unwrap().to_str().unwrap();
		let name = file.derva_str(save[6]).unwrap().to_str().unwrap();
		let offset = save[7];
		cvars.push(ConVar { dll_name, name, desc, default_string, offset, flags, min_value, max_value });
	}
