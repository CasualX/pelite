use pelite::Error::Null;

use crate::wasm::*;
use crate::PeFile;

#[unsafe(no_mangle)]
pub unsafe fn pefileDosHeader(pefile: *mut PeFile) { unsafe {
	let dos_header = (*pefile).as_ref().dos_header();
	set_json(dos_header);
}}
#[unsafe(no_mangle)]
pub unsafe fn pefileDosImage(pefile: *mut PeFile) { unsafe {
	let dos_image = (*pefile).as_ref().dos_image();
	set_bytes(dos_image);
}}
#[unsafe(no_mangle)]
pub unsafe fn pefileNtHeaders(pefile: *mut PeFile) { unsafe {
	let nt_headers = (*pefile).as_ref().nt_headers();
	set_json(nt_headers);
}}
#[unsafe(no_mangle)]
pub unsafe fn pefileFileHeader(pefile: *mut PeFile) { unsafe {
	let file_header = (*pefile).as_ref().file_header();
	set_json(file_header);
}}
#[unsafe(no_mangle)]
pub unsafe fn pefileOptionalHeader(pefile: *mut PeFile) { unsafe {
	let optional_header = (*pefile).as_ref().optional_header();
	set_json(optional_header);
}}
#[unsafe(no_mangle)]
pub unsafe fn pefileDataDirectory(pefile: *mut PeFile) { unsafe {
	let data_directory = (*pefile).as_ref().data_directory();
	set_json(data_directory);
}}
#[unsafe(no_mangle)]
pub unsafe fn pefileSectionHeaders(pefile: *mut PeFile) { unsafe {
	let section_headers = (*pefile).as_ref().section_headers();
	set_json(section_headers);
}}
#[unsafe(no_mangle)]
pub unsafe fn pefileHeaders(pefile: *mut PeFile) { unsafe {
	let headers = (*pefile).as_ref().headers();
	set_json(headers);
}}

#[unsafe(no_mangle)]
pub unsafe fn pefileRichStructure(pefile: *mut PeFile) { unsafe {
	match (*pefile).as_ref().rich_structure() {
		Ok(rich_structure) => set_json(rich_structure),
		Err(Null) => set_null(),
		Err(err) => set_error(err),
	}
}}
#[unsafe(no_mangle)]
pub unsafe fn pefileExports(pefile: *mut PeFile) { unsafe {
	match (*pefile).as_ref().exports() {
		Ok(exports) => set_json(exports),
		Err(Null) => set_null(),
		Err(err) => set_error(err),
	}
}}
#[unsafe(no_mangle)]
pub unsafe fn pefileImports(pefile: *mut PeFile) { unsafe {
	match (*pefile).as_ref().imports() {
		Ok(imports) => set_json(imports),
		Err(Null) => set_null(),
		Err(err) => set_error(err),
	}
}}
#[unsafe(no_mangle)]
pub unsafe fn pefileLoadConfig(pefile: *mut PeFile) { unsafe {
	match (*pefile).as_ref().load_config() {
		Ok(load_config) => set_json(load_config),
		Err(Null) => set_null(),
		Err(err) => set_error(err),
	}
}}
#[unsafe(no_mangle)]
pub unsafe fn pefileTls(pefile: *mut PeFile) { unsafe {
	match (*pefile).as_ref().tls() {
		Ok(tls) => set_json(tls),
		Err(Null) => set_null(),
		Err(err) => set_error(err),
	}
}}
#[unsafe(no_mangle)]
pub unsafe fn pefileDebug(pefile: *mut PeFile) { unsafe {
	match (*pefile).as_ref().debug() {
		Ok(debug) => set_json(debug),
		Err(Null) => set_null(),
		Err(err) => set_error(err),
	}
}}
