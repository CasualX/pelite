...
pub fn parse(value: &str) -> std::result::Result<RvaRange, String> {
...
fn parse_rva(value: &str) -> std::result::Result<u32, String> {
...
mod tests {
	...
	fn parses_hexadecimal_rva_ranges() {
		let bare = parse("1000..10ff").unwrap();
		assert_eq!((bare.start, bare.end), (0x1000, 0x10ff));
		let prefixed = parse("0x20..0X30").unwrap();
	...
	fn rejects_invalid_rva_ranges() {
		...
		assert!(parse("1000..1000").is_err());
		assert!(parse("2000..1000").is_err());
		assert!(parse("xyz..1000").is_err());
	}
}
